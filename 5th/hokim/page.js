...
function createBtn() {
    ...
    var innerText =  `<div><p> 이름 </p><input type="text" id="name" name="name" required minlength="2" maxlength="8" size="10">
        <p> 비밀번호 </p><input type="number" id="password" name="password" required minlength="4" maxlength="4" size="10">
        </div>
    ...
    function onclick() {
        ...
        button.addEventListener("click", () => {
            ...
            deleteAll_child(box);
            box.innerHTML = `<p>${name}님의 계좌번호는 [${mybank.createAccount(name, password)}] 입니다.</p>
...
function depositBtn() {
    ...
    var innerText = `<div><p> 계좌번호 </p><input type="number" id="accountId" name="accountId" required minlength="2" maxlength="8" size="10">
        <p> 비밀번호 </p><input type="number" id="password" name="password" required minlength="4" maxlength="4" size="10">
        <p> 입금할 금액 </p><input type="number" id="money" name="money" size="10">
        </div>
    ...
    function onclick()
    {
        ...
        if (!mybank.depositAccount(accountId, money, password)) {
            box.innerHTML = `<p>에러가 발생했습니다.</p>
            <button id="btn"> 확인</button>`;
        }
        else {
            box.innerHTML = `<p>
            ${mybank.getName(accountId)}님의 계좌로 ${parseInt(money)}원을 입금했습니다.<br/>
            ${mybank.getName(accountId)}님의 계좌 잔고는 [${parseInt(mybank.lookupAccount(accountId, password).balance)}]원 입니다.</p>
            <button id="btn"> 확인</button>`;
        }
        var button = document.getElementById("btn")
        button.addEventListener("click", () =>
...
function withdrawBtn() {
    ...
    var innerText = `<div><p> 계좌번호 </p><input type="number" id="accountId" name="accountId" required minlength="2" maxlength="8" size="10">
        <p> 비밀번호 </p><input type="number" id="password" name="password" required minlength="4" maxlength="4" size="10">
        <p> 출금할 금액 </p><input type="number" id="money" name="money" size="10">
        </div>
...
function remitBtn() {
    ...
    var innerText = `<div><p> 계좌번호 </p><input type="number" id="fromId" name="fromId" required minlength="2" maxlength="8" size="10">
        <p> 비밀번호 </p><input type="number" id="password" name="password" required minlength="4" maxlength="4" size="10">
        <p> 송금할 계좌번호 </p><input type="number" id="toId" name="toId" required minlength="2" maxlength="8" size="10">
        <p> 금액 </p><input type="number" id="money" name="money" size="10">
        </div>
    ...
    function onclick()
    {
        ...
        if (!mybank.remitMoney(fromId, toId, money, password)) {
            ...
            ${account.name}님의 계좌 잔고는 [${parseInt(account.balance)}]원 입니다.</p>
            <button id="btn"> 확인</button>`;
        }
        var button = document.getElementById("btn")
        button.addEventListener("click", () =>
